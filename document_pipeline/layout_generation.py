...
from utils.count_bbox_size import get_font_vmetrics_pt
    ...
    def layout_words(
        ...
        *,
        ...
        def _fix_vertical_overlaps(
            ...
            If upper_bottom > lower_top (overlap), set boundary = (upper_bottom + lower_top) / 2.
            Then:
            - For all words in upper line: y2 := min(y2, boundary)
            - For all words in lower line: y1 := max(y1, boundary)

            This makes the lower boundary of the upper line and the upper boundary of the lower line
            equal to the arithmetic mean, as requested.
            """
            ...
            for i in range(len(lines) - 1):
                ...
                boundary = 0.5 * (upper_bottom + lower_top)
